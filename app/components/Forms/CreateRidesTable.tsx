...
function ridesToCSV(rides: any[]): string {
      const headers = ["Passenger", "Pickup", "Dropoff", "Driver", "Status", "Date"];
...
export default function CreateRidesTable({ rides, totalCount, totalPages, currentPage }: CreateRidesTableProps) {
                  ...
                  <table className="w-full border-collapse bg-white text-left text-sm text-gray-500">
                        <thead className="bg-gray-50">
                              <tr>
                                    ...
                                    <th className="px-6 py-4 text-center font-medium text-gray-900">Driver</th>
                                    <th className="px-6 py-4 text-center font-medium text-gray-900">Status</th>
                              </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 border-t border-gray-100">
                              {rides?.map((ride: any, index: number) =>
                                    <tr key={index}>
                                          ...
                                          <td className={`text-center ${ride.status === "Completed" ? 'bg-green-100' : ride.status === "Cancelled" ? 'bg-red-100' : 'bg-gray-100'}`}>{ride.status}</td>
                                    </tr>
                              )}
                        </tbody>
                  </table>
                  ...
                  {totalPages > 1 && (
                        <div className="flex items-center justify-between mt-4 px-2">
                              <p className="text-sm text-gray-600">
                                    Showing {startIndex + 1}-{Math.min(startIndex + ITEMS_PER_PAGE, totalCount)} of {totalCount} results
                              </p>
